...
const TOOLS = [
  { key: "salary", nav: "What is your salary worth?",
    blurb: "A salary that has not changed since a year you remember, priced in today's money." },
  { key: "savings", nav: "What happened to your savings?",
    blurb: "Pounds kept as pounds against the same pounds swapped for dollars on day one." },
  { key: "dollar", nav: "What was a dollar worth that day?",
    blurb: "The CBE rate on any date since 2005, and on the same date in every year since." },
];
...
function toolCrumbs(key) {
  ...
  return crumbs([{ label: "Work it out", href: "#/tools" }, { label: tool ? tool.nav : "Tools" }]);
}
...
function otherTools(current) {
  return (
    '<div class="pair">' +
    TOOLS.filter((t) => t.key !== current).map((t) =>
      '<a class="wide-card" href="#/tools/' + t.key + '">' +
      '<p class="eyebrow">Work it out</p>' +
      "<h3>" + esc(t.nav) + "</h3><p>" + esc(t.blurb) + "</p>" +
      '<span class="go">Try it →</span></a>').join("") +
    "</div>"
  );
}
...
const CPI_CAVEAT =
  '<details class="foot-note tool-note"><summary>Where these numbers come from, and where they are soft</summary>' +
...
function viewTools() {
  document.getElementById("app").innerHTML =
    ...
    '<p class="eyebrow">Work it out</p>' +
    "<h2>Three questions about your own money</h2>" +
    '<p class="lede">The same numbers as the rest of the site, asked in the second person. ' +
    "Nothing you type leaves your browser: the arithmetic happens on this page, and the only " +
    "thing that travels is the link, which carries what you typed so you can send someone the " +
    "answer rather than the form.</p>" +
    '<div class="topic-grid">' +
    TOOLS.map((t) =>
      '<a class="topic-card" href="#/tools/' + t.key + '">' +
      '<span class="ico" aria-hidden="true">' +
      ({ salary: "◷", savings: "◈", dollar: "$" }[t.key] || "?") + "</span>" +
      "<h3>" + esc(t.nav) + "</h3><p>" + esc(t.blurb) + "</p></a>").join("") +
    "</div>" +
    "</section>" +
...
async function viewSalary(query) {
  ...
  if (amount && then) {
    ...
    answer =
      ...
      '<p class="eyebrow">' + esc(niceDate(then[0]).replace(/^\d+ /, "")) + " to " +
      esc(niceDate(now[0]).replace(/^\d+ /, "")) + "</p>" +
      "<h2>" + esc(money(amount)) + " then is " + esc(money(worth)) + " now</h2>" +
      '<p class="lede">A salary of ' + esc(money(amount)) + " that never changed has lost <b>" +
      pct(lost) + "%</b> of what it could buy. To be worth what it was, it would have to be <b>" +
      esc(money(needed)) + "</b> today.</p>" +
      ...
      "<span>what " + esc(money(amount)) + " from " + esc(niceDate(then[0])) +
      " buys, in the money of the day</span></div>" +
      '<div class="controls"><button class="chip solid" id="tool-share">Copy a link to this answer</button></div>' +
      "</div></section>";
  }
  ...
  app.innerHTML =
    ...
    '<section class="section">' +
    '<p class="eyebrow">Work it out</p>' +
    "<h2>What is your salary worth?</h2>" +
    '<p class="lede">Put in a salary and the month you were earning it. This prices the same ' +
    "number in today's money, using CBE's own inflation.</p>" +
    toolForm([
      { id: "amount", label: "Monthly salary, in pounds", type: "number", value: amount || "",
        placeholder: "8000", min: "1", step: "any" },
      { id: "from", label: "Which month", type: "month", value: from ? from.slice(0, 7) : "",
        min: firstMonth, max: lastMonth },
    ], "Work it out") +
    '<p class="foot-note">The index runs from ' + esc(niceDate(obs[0][0])) + " to " +
    esc(niceDate(now[0])) + ".</p>" +
    "</section>" + answer +
...
async function viewSavings(query) {
  ...
  if (amount && priceThen && fxThen) {
    ...
    answer =
      '<section class="section band"><div class="wrap-inner">' +
      '<p class="eyebrow">' + esc(niceDate(from)) + " to " + esc(niceDate(fxNow[0])) + "</p>" +
      "<h2>" + esc(money(amount)) + " kept as pounds now buys " + esc(money(keptReal)) + " of " +
      esc(niceDate(from).replace(/^\d+ /, "")) + " shopping</h2>" +
      '<p class="lede">Prices have risen <b>' + (Math.round(inflation * 100) / 100) +
      "×</b> since then, so the cash lost <b>" + pct(lost) + "%</b> of what it could buy. " +
      "The same money swapped for dollars that day would be <b>$" +
      Math.round(dollars).toLocaleString("en-US") + "</b>, which is " + esc(money(inDollarsNow)) +
      " today, and buys " + esc(money(inDollarsReal)) + " of that same shopping.</p>" +
      '<table class="indicators"><thead><tr><th>What you did with it</th>' +
      "<th>Pounds today</th><th>What it buys</th></tr></thead><tbody>" +
      "<tr><td class='name'>Kept it as pounds</td><td>" + esc(money(amount)) + "</td><td>" +
      esc(money(keptReal)) + "</td></tr>" +
      "<tr><td class='name'>Swapped it for dollars on day one</td><td>" + esc(money(inDollarsNow)) +
      "</td><td>" + esc(money(inDollarsReal)) + "</td></tr>" +
      "</tbody></table>" +
      '<p class="foot-note">"What it buys" is in the money of ' + esc(niceDate(from)) +
      ", so both rows can be compared with the number you started from. " +
      "The dollar row is the CBE official selling rate on both dates and ignores what a bank " +
      "would have charged you on either side of it.</p>" +
      '<div class="controls"><button class="chip solid" id="tool-share">Copy a link to this answer</button></div>' +
      "</div></section>";
  }
  ...
  app.innerHTML =
    ...
    '<section class="section">' +
    '<p class="eyebrow">Work it out</p>' +
    "<h2>What happened to your savings?</h2>" +
    '<p class="lede">Put in an amount and the day you set it aside. This says what it still ' +
    "buys, and what the same money would have been worth had you swapped it for dollars that " +
    "day and done nothing else.</p>" +
    toolForm([
      { id: "amount", label: "Amount, in pounds", type: "number", value: amount || "",
        placeholder: "10000", min: "1", step: "any" },
      { id: "from", label: "Set aside on", type: "date", value: from,
        min: firstDay, max: lastDay },
    ], "Work it out") +
    '<p class="foot-note">Any date from ' + esc(niceDate(firstDay)) + " onwards.</p>" +
    "</section>" + answer +
    '<section class="section"><div class="band-inset">' + CPI_CAVEAT + "</div>" +
...
async function viewDollarOn(query) {
  ...
  app.innerHTML =
    ...
    '<section class="section">' +
    '<p class="eyebrow">Work it out</p>' +
    "<h2>What was a dollar worth that day?</h2>" +
    '<p class="lede">A birthday, a wedding, the day you started a job. CBE has published an ' +
    "official rate every business day since January 2005.</p>" +
    toolForm([{ id: "on", label: "Pick a date", type: "date", value: asked,
                min: firstDay, max: lastDay }], "Look it up") +
    "</section>" +

    (hit
      ? '<section class="section band"><div class="wrap-inner">' +
        '<p class="eyebrow">' + esc(regime ? regime.label : "CBE official selling rate") + "</p>" +
        "<h2>" + esc(fmt(hit[1], fx.unit)) + " on " + esc(niceDate(hit[0])) + "</h2>" +
        '<p class="lede">' +
        (hit[0] !== asked
          ...
          : "") +
        (multiple
          ? "A dollar costs <b>" + (Math.round(multiple * 100) / 100) + "×</b> more today, at " +
            esc(fmt(now[1], fx.unit)) + ". Put the other way round, " + esc(money(1000)) +
            " was $" + Math.round(1000 / hit[1]).toLocaleString("en-US") + " then and is $" +
            Math.round(1000 / now[1]).toLocaleString("en-US") + " now."
          : "") +
        "</p>" +
        '<div class="controls"><button class="chip solid" id="tool-share">Copy a link to this answer</button>' +
        '<a class="chip" href="#/s/' + USD + '">The whole series</a></div>' +
        "</div></section>"
      : "") +
